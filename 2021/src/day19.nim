import helpers, std/[sequtils, strscans, strutils, sets, tables]
...
proc alignAll(data: Data): Aligns =
...
proc readInputData(): Data =
  for chunk in readInputString(day = 19).strip.split("\n\n"):
    result.add chunk.parseReport

proc solveDay19*: IntSolution =
  let data = readInputData() 
  result.part1 = data.alignAll.beacons(data).card
  result.part2 = data.alignAll.scanners.maxDistance
